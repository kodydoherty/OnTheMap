...
class StudentClient: NSObject {
    ...
    func taskForUdacityGETMethod(method: String, completionHandler: (result: AnyObject!, error: NSError?) -> Void) -> NSURLSessionDataTask {
        /* 2/3. Build the URL and configure the request */
        let urlString = Constants.AuthorizationURL + method + "/" + self.userID!
        let url = NSURL(string: urlString)!
    ...
    // Parse Posts
    func taskForPOSTParseMethod(base: String, method: String, jsonBody: [String:AnyObject], completionHandler: (result: AnyObject!, error: NSError?) -> Void) -> NSURLSessionDataTask {
